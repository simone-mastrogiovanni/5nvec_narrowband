function [gdout g0 g45 iok sbl_ dt0all phall phall1]=sfdb09_band2sbl3_sergio(freq,cont,folder,pialist,filesbl,simpar,dtout)
...
%disp(sprintf('Dt0=%f tdt=%f t1=%f t2=%f t3=%f dtsd1=1.10%e dtsd2=%e dtsd3=%e\n',Dt0,tdt2tdb(mjd),tt(1),tt(length(tt)/2),tt(length(tt)/2),Dtspin(1),Dtspin(length(Dtspin)/2),Dtspin(length(Dtspin)/2)));
%fprintf(fidfile,'%f %f %8.10f %8.10f %2.10e %2.10e %f \n',Dt0,tdt2tdb(mjd),tt(1),tt(length(tt)/2),Dtspin(1),Dtspin(length(Dtspin)/2),gmst(mjd));
%disp(sprintf('%e %e\n',df0*pos(1)^2/2,df0*tt(1)*pos(1)));
