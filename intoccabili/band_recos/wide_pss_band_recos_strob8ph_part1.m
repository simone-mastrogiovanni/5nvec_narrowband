function wide_pss_band_recos_strob8ph_part1(path,name,ant_sour,ext1,sourcestr, filein,dtout,tref,ioutin,gdname,tint)
...
%                         save also the other components of
...
%               - Check the string conversion, must be very precise (SM)
...
%
% NB KEEP AN EYE ON THE PRECISION OF THE num2str(CONVERSION), PUT THE MAX PRECISION  IN THE CONVERSION
...
% if exist('vetos','var')
%    fvetos=fopen(vetos,'r')
%    locpeak=textscan(fvetos,'%f %f','Delimiter','\t');
%    disp(sprintf('The Following frequency will be removed \n'));
%    freq_veto=locpeak{1}
%    width_veto=locpeak{2}
%    fclose(fvetos);
% end
...
lfftout0=lfftin*sfdb09_us.tsamplu*round(1/dtout)
[d lfftout]=divisors_snag(lfftin,lfftout0);
lfftout=lfftout0; % CONTROLLARE SE VA BENE
fprintf(fidlog,'lfftout = %f \n',lfftout);

ibias=1000;
lentot=ceil(min(maxt-sbl_.t0,tint(2)-tint(1))*86400+dtfft+2*ibias)*round(1/dtout)
%save('provalook.mat')
g=zeros(lentot,1);
gtime=zeros(lentot,1);
g_p=zeros(lentot,1);
g_tnot=zeros(lentot,1);
...
%     [out outime iout ii tchsupp p out_t_not out_p]=pss_fd_resamp8ph_nb(t1,kt,freq_veto,width_veto);
%     end
    %if ~exist('freq_veto','var')
	[out outime iout ii tchsupp p out_t_not out_p]=pss_fd_resamp8ph_nb(t1,kt);	
    %end
   ...
   g_tnot(ioutb:ioutb+length(P)-1)=out_t_not;
   g_p(ioutb:ioutb+length(P)-1)=out_p;
...
save([path,name,sourcestr,'_timevar','.mat'],'sii','stch','siout','gtime','g_tnot','g_p');
