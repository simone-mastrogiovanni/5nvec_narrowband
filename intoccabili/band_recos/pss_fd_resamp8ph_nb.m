function [out outime iout ii tchsupp p out_t_not out_p]=pss_fd_resamp8ph_nb(tin,kt,enh)
...
%
%  OUTPUT:
%  out: Output time series down-sampled 
%  outime: Output time vector down-sampled as out
%  iout: Start point of the short FFT in second
%  ii: Indexes of the down-sampled data-points
%  tchsupp: End point of the short FFT in second
%  p: Last down-sampled time series, needed for the length
%  out_t_not: Time vector not corrected for Doppler effect out_p
%  out_p: Vector containing Doppler correction in time
%
%  Conventions:
...
%%%%% BUILD THE TIME SERIES FROM THE SFDB %%%%%%%%
switch in.op
    case 1 % the first n2
        %in.empdelay=0;
        toff=0-in.empdelay;
...
%         if exist('freq_veto','var')
%             for vcoun=1:1:length(freq_veto)
%                 down_s=round((freq_veto(vcoun)-0.5*width_veto(vcoun))/in.step_simo)+1;
%                 up_s=round((freq_veto(vcoun)+0.5*width_veto(vcoun))/in.step_simo)+1;
%                 infft(down_s:up_s)=0;
%             end
%         end
...
%         tt=tch+p+dtsd+in.Dteinst;
        ...
        t_not_corr=tt1+toff+in.dtin1*(0:n2-1);
...
%         tt=tch+p+dtsd+in.Dteinst;
        ...
        [out1 outime1 iout1 ii]=strobo([in.last support0(n2+1:n2+n4) support1(n4+1:n2)],[tch(n2) tch(1:n4) tch(n4+1:n2)],tch,in.dtout,toff);
        out=out1;
        outime=outime1;
        out_t_not=t_not_corr(ii+1);
        out_p=p(ii+1);
...
%         tt=tch+toff+p+dtsd+in.Dteinst;
        ...
        [out1 outime1 iout1 ii]=strobo(support0(n2+1:n2*2),tch,tch,in.dtout,toff);
        out_t_not=t_not_corr(ii+1);
        out_p=p(ii+1);
        out=out1;
        outime=outime1;
...
p=p0+v0*x;
 ...
 %$Resampling function
function [out outime iout ii]=strobo(x,y,tt,dtout,toff)
tt=tt/dtout;
tt1=floor(tt);
ii=find(diff(tt1));
outime=y(ii+1);
