function t_gpu=reduce_pss_band_recos_strob8ph_part2_v2_gpu(secondtime,selpath,timepath,name2,inmat,inseg,fcenter,dext2,sdindex_min,sdindex_max,cut_array,step)
...
clear iii
    ...
    gpu_time_vector=[86400*5,86400*10,86400*20,86400*40,86400*60,86400*70,86400*80,86400*90]; 
    for i_gpu=1:1:8
        i_gpu
    
    x=1:1:gpu_time_vector(i_gpu);
    y=x.';
    x=y;
    y_original=(y);
    x_original=(x);
...
for count_sd=sdindex_min:1:sdindex_max
    ...
    [gg_sc_sd,y_sd]=simone_applies_spin_down_corrections_gpu(y,x,gg_sc,count_sd,dtout);  % Apply spin-down corrections
    ...
    clearvars -except i_gpu t_gpu gpu_time_vector y_sd gg_sc x y_original x_original gg_sc_sd  secondtime cut_array fcenter dext2 t0 ext1 gg_sc source ant in ext2 freqint thr sourcestr selpath inmat inseg sdindex_min sdindex_max dec_index step count_sd name2 df0new timepath dtout freqint ant sourcestr source
    if step~=0
        sourcestr=['divided_in_',num2str(step),'Hz_',sourcestr];        
    end
    
    [Sfft Sfftshifted  hvectors fra  info gg_sc_clean tcount]=narrow_pentav_gpu(y_sd,x,selpath,gg_sc_sd,source,ant,ext2,freqint,sourcestr);
    close all
   gpuDevice(1)
    t_gpu(i_gpu)=tcount;
    continue
    ...
    clearvars -except ext1 gg_sc secondtime ant t0 timeshift_simo cut_array selpath inmat inseg sdindex_min sdindex_max y_original x_original  dec_index step count_sd name2 fcenter dext2 sdstart timepath dtout freqint source sourcestr in
    close all
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
    end
