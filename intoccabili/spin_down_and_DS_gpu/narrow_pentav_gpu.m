function [Sfft Sfftshifted hvectors fra1 info gg_sc_clean tcount]=narrow_pentav_gpu(y,t,selpath,gg_sc_clean,sour,ant,ext2,freqint,sourcestr)
...
% Input arguments:
% y: Data strain (MATLAB gpu FORMAT)
% t: Data strain (MATLAB GPU FORMAT)
...
%fftw('dwisdom',[]); % implement the fftw
...
if exist([selpath,num2str(ext2(1)+freqint),'_',num2str(ext2(2)+freqint),'_',sourcestr,'_templates.mat'],'file')
    load([selpath,num2str(ext2(1)+freqint),'_',num2str(ext2(2)+freqint),'_',sourcestr,'_templates.mat'],'pentaAplusfft','pentaAcrossfft');
...
if ~exist([selpath,num2str(ext2(1)+freqint),'_',num2str(ext2(2)+freqint),'_',sourcestr,'_templates.mat'],'file')
    ...
    pentaAplusfft=(pentaAplusfft);
    pentaAcrossfft=(pentaAcrossfft);
    
    Ap2=(norm(pentaAplusfft))^2;
    Ac2=(norm(pentaAcrossfft))^2;
    %Save the sidereal 5-vectors
    save([selpath,num2str(ext2(1)+freqint),'_',num2str(ext2(2)+freqint),'_',sourcestr,'_templates.mat'],'pentaAplusfft','pentaAcrossfft');
end
Ap2=(Ap2);
Ac2=(Ac2);
pentaAplusfft=(pentaAplusfft);
...
hplusfft=(hplusfft)/Ap2;
hcrossfft=(hcrossfft)/Ac2;

hplusfftshifted=(hplusfftshifted)/Ap2;
hcrossfftshifted=(hcrossfftshifted)/Ac2;
