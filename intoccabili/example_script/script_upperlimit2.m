...
selpath_H='/storage/users/simone/ligoh_O1_cal/vela/sdindex/'; % Metti selpath H, stessa dove sono gli originali Cancella i selection
...
f_cent_H=num2str(22.3740); % Metti freq centrale, stessa originali
f_band_H=num2str(0.05); % Metti banda esplorata, stesstmp_upperlimit_L1.mata originale

selpath_L='/storage/users/simone/ligol_O1_cal/vela/sdindex/'; % Metti selpath L, stessa dove sono gli originali Cancella i selection
...
timepath_H='/storage/users/simone/ligoh_O1_cal/vela/C01H_vela_vela_timevar.mat'; % metti file temporale per H
mainfile_H='/storage/users/simone/ligoh_O1_cal/vela/C01H_vela_veladata_part1.mat'; % file principale per H
timepath_L='/storage/users/simone/ligol_O1_cal/vela/C01L_vela_vela_timevar.mat'; % metti file temporale per L
mainfile_L='/storage/users/simone/ligol_O1_cal/vela/C01L_vela_veladata_part1.mat'; % metti file principale per L


Ninj=100; % numero di onde che vuoi iniettare
Npk=499; % numeri di pk da iniettare, uguali al reduction
...
for i=1:1:Ninj 
    ...
    pars.t0=t0_H;
    save('tmp_upperlimit_H1.mat','in','ibias','ext1','ioutb','g','dtout','t0','pars','ioutout','T0','P','out','source','ant','sourcestr');
    if exist('ioutin','var')
    save('tmp_upperlimit_H1.mat','in','ibias','ext1','ioutb','g','dtout','t0','pars','ioutout','T0','P','out','source','ant','sourcestr','ioutin');
    else
            save('tmp_upperlimit_H1.mat','in','ibias','ext1','ioutb','g','dtout','t0','pars','ioutout','T0','P','out','source','ant','sourcestr');
    ...
    reduce_for_upper(selpath_H,timepath_H,['injection_H_n_',num2str(i),'_'],'tmp_upperlimit_H1.mat',inseg_H,f_cent_H,f_band_H,num2str(0),num2str(0),cut_arr_H,num2str(0),num2str(0));
    ...
    pars.t0=t0_L;
    if exist('ioutin','var')
    save('tmp_upperlimit_L1.mat','in','ibias','ext1','ioutb','g','dtout','t0','pars','ioutout','T0','P','out','source','ant','sourcestr','ioutin');
    else
            save('tmp_upperlimit_L1.mat','in','ibias','ext1','ioutb','g','dtout','t0','pars','ioutout','T0','P','out','source','ant','sourcestr');
    ...
    reduce_for_upper(selpath_L,timepath_L,['injection_L_n_',num2str(i),'_'],'tmp_upperlimit_L1.mat',inseg_L,f_cent_L,f_band_L,num2str(0),num2str(0),cut_arr_L,num2str(0),num2str(0));
    inj_wave{i}=wave;
    i
end
save('inj_waves_file_vela.mat','inj_wave');
