function f=radpat_interf(source,antenna,tsid)
%RADPAT_INTERF  (energy) interferometric detector response to a source
%
...
f3=c2phi.*s2psi;
ftheta=.5*(1.+ctheta.^2);
f2_circ=.5*(1-eps)*((ftheta.*c2phi).^2+f2.^2);
f2_lin=eps*(ftheta.*f1-f2.*s2psi).^2;
f=f2_circ+f2_lin;