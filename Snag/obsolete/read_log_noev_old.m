function out=read_log_noev(par,file)
%READ_LOG_NOEV  reads pss log files without events
%
...
while 1
    ...
    if len > 4
        if strcmp(line(1:3),'>>>')
            kstat=kstat+1;
            out.stat(kstat).type=line(5:7);
            out.stat(kstat).pars=sscanf(line(10:len),'%f');
