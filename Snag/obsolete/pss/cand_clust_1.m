function [clust, serv]=cand_clust_1(A,dist,dfr,dsd)
...
%     end
    ii=i+1:serv(1,i);
    d=distance(A(:,i),A(:,ii),dfr,dsd);
    iiii=find(d <= dist); 
    iii=find(serv(2,iiii)== 0);
    if length(iii)==length(iiii)
        serv(2,ii(iiii))=i;
        serv(2,i)=i;
        tot1=tot1+1;
    else
        iii=find(serv(2,ii(iiii)));
        jj=intersect(iii,ii(iiii));
      if length(jj>0)
        M=min(serv(2,ii(jj)));
        serv(2,ii(iiii))=M;
        serv(2,i)=M;
