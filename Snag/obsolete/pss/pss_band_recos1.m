function [g vv pp tim par1 par2 par3]=pss_band_recos1(source,filein,nn0,gdname,tint)
...
%    filein    input file
%    nn0       imposed reduced fft length (a sub-multiple of original fft length)
...
%         pos3=p(3)+v(3)*dt*((0:n2-1)-n2/2);
        pos1=p(1)+v(1)*dt*((1:n2)-n2/2); % computed at sample n/2
        pos2=p(2)+v(2)*dt*((1:n2)-n2/2);
        pos3=p(3)+v(3)*dt*((1:n2)-n2/2);
        pos=pos1*r(1)+pos2*r(2)+pos3*r(3);
        
        tt=T1+dt*((0:n2-1)); % T1 block time respect to t0
%         tt=dt*((0:n2-1)-n2/2); % time respect to the beginning of the block
        ph1=mod((df0*(tt.^2)/2+ddf0*(tt.^3)/6)*2*pi,2*pi); % spin-down correction to t0 frequency (apparent frequency)
        ph2=mod(fr0*pos*2*pi,2*pi);  % fr0 recomputed for each block
        phi=ph1+ph2;
        par1=[par1 ph1];
        par2=[par2 ph2];
        par3=[par3 pos];
