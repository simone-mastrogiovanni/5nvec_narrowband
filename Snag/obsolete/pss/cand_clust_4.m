function [clust, serv, PP, II, jII, out]=cand_clust_4(A,dist,dfr,dsd)
...
%     ii=i+1:serv(1,i);
    ...
    iiii=find(d <= dist);%sprintf('%d %d \n',length(ii),length(iiii))
    liiii=length(iiii);
        isnand=isnan(d);
        kk=find(isnand);
    if length(kk) > 0
...
for i = 1:n1
    if i == floor(i/10000)*10000
        s=datestr(now);
        fprintf('f = %f  %d %d %d %d %s \n',A(1,i),i,icount,icount0,icount1,s)
    end
    ii=II(jII(i):jII(i+1)-1);
    nn(i)=length(ii);
    if PP(i) == -1
        PP=arraffa(i,i,ii,jII,II,PP);
    end
end
toc
...
function PP=arraffa(progenitor,parent,ii,jII,II,PP)

global icount icount0 icount1 % jII II PP
icount=icount+1;
if progenitor ~= parent
    icount0=icount0+1;
end
% parent,icount,ii,jII(ii)
...
%         icount0=icount0+1;
    ...
    otherwise
        PP(ii(1))=progenitor;
        for i = ii(2:lii)
            iiout=II(jII(i):jII(i+1)-1);
            if length(iiout) > 0
                if PP(i) == -1
                    PP(i)=progenitor;
                    PP=arraffa(progenitor,i,iiout,jII,II,PP); % conquers the "near"
                end
            end
        end
end

% PP(ii)=parent; % puts the flag