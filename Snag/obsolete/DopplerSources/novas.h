/*
   NOVAS-C "WITH ADDITIONS BY E.Majorana, spring 1999"
...
/*
   function prototype descriptor for Fortran subroutine 'pleph'.

 */
extern void __stdcall PLEPH(double *tjdt,long int *targ,
 	                            long int *cen,double *posvel
 								/*,long int *err_flg*/); 
 
 

/*
   TEMPORARY FUNCTION TO CHECK RESULTS (by S. FRASCA)

   void Doppler_sergio(double time,
                       fk5_entry *stars,
                       site_info *location, 
                       double *delta_sergio);
*/
...
//  Novas_driver (per la console appl) e Doppler (per fare il mex file) 
 
   
 
 void Novas_driver(FILE *out1,site_info *location,double *poss, double *vels,
                     double dotu,int year,double *deltaf);


 
 void Dopplerem(fk5_entry * stars,site_info *location,
              double dotu,int year,double fmark,double *fmark_e,
			  double *posid, double *velod);
//////////////////////////////////////////////////////////////
