...
% Example: [PosConst]=constellation([1 1;2 2]);
% Reliable: 2
%----------------------------------------------------------------------------
RAD = 180./pi;
...
if (ColN==2),
   % do nothing
elseif (ColN==7),
   RA  = convertdms(Coo(:,1:3),'H','r');
   Dec = convertdms(Coo(:,4:7),'D','R');
   Coo = [RA, Dec];
else
...
[RA1, RA2, Dec1, Const] = textread(ConstFileName,'%f %f %f %s','commentstyle','matlab');
...
for I=1:1:N,
   ...
   [Min, MinInd] = min(Coo(I,2) - Dec1(CI));
