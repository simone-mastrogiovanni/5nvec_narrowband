...
/* Check for proper number of arguments. */
  if (nrhs != 4) {
    mexErrMsgTxt("Four inputs required: socket, VectName, Tstart, Duration");
  } else if (nlhs > 3) {
...
/* evaluate the string lenght */
 ...
 Tstart = (double) mxGetScalar(in_Tstart);
 Tend   = (double) mxGetScalar(in_Tend);
 
 
 if ((ans=FrvSFileIGetV(fd, Vect_name, Tstart, Tend)) == NULL ) 
    { 
      printf("Error reading remote vector\n");
      return;
     }
