function sds_resam_reshapeA(listin,maxndat,frout,nsec,folderin,folderout)
...
%
%   sds_resam_reshape(listin,maxndat,frout,nsec,folderin,folderout)
%
%   listin      list of input file (in a text file, typically in the input folder)
%               "no" -> single file ; to create the list: dir /b > list.dat
...
for i = 1:nfiles
    ...
    if i == 1
        ...
        if abs(t1-t1next) > dt/86400 % secondary close condition
            fprintf('hole of %f hours between %s and %s \n',(t1-t1next)*24,mjd2s(t1next,0),mjd2s(t1,0))
            fileout1=fileout;
            t2=t1;
            fileout=new_file(file{i},t2);
            ic_newstream=1;
    ...
    while r_in > 0
        ...
        if r_intot >= ndat1*nch
            ...
            rest=rest-w_out;
            if rest <= 0 % primary close condition
                fileout1=fileout;
