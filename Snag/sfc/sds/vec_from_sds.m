function [vec,sds_,tim0,holes,n_nan]=vec_from_sds(sds_,chn,len,alpers)
%VEC_FROM_SDS  fills vec with data from sds files 
%
%    [vec,sds_,tim0,holes,n_nan]=vec_from_sds(sds_,chn,len,alpers)
% 
%    Rules:
%
...
while len1 > 0
    ...
    if nread > 0
        vec(kvec:len2)=A(:,chn); % ATTENZIONE ! ERRORE PER MOLTI CANALI
        kvec=len2+1;
    end
    ...
    if len1 > 0
        ...
        if strcmp(sds_.filspost,'#NOFILE')
            ...
            holes.nzeros(holes.nholes)=len-kvec+1;
            holes.nztot=holes.nztot+holes.nzeros(holes.nholes);
            holes.kzeros(holes.nholes)=kvec;
            holes.end=1;
            n_nan=check_nan(vec);
