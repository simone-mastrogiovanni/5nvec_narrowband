function [gdout g0 g45 iok sbl_ dt0all phall phall1]=sfdb09_band2sbl2noeinst(freq,cont,folder,pialist,filesbl,simpar,dtout)
...
%                         st=gmst(mjd)+dtori*(0:lfftori-1)/3600+antenna.long/15;
                        st=gmst(mjd)+dtori*(86400/SD)*(86400/SD)*(0:lfftori-1)/3600;
                        i1=mod(round(st*(nsid-1)/24),nsid-1)+1;

                        e0=exp(1j*ph);
                        sim=simamp*real((Hp*sid1(i1)+Hc*sid2(i1)).*e0);
                        stsub=gmst(mjd)+dtori*(86400/SD)*(86400/SD)*(0:nsampsec:lfftori-1)/3600;
