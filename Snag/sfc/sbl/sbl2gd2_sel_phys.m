function g2=sbl2gd2_sel_phys(file,chn,x,y)
...
if ~exist('x')
    answ=inputdlg({'Initial primary abscissa value (time)',...
            'Final primary abscissa value',...
            'Mean every',...
            'Initial secondary abscissa value',...
            'Final secondary abscissa value', ...
            'Mean every'},...
            'Abscissa selection',1,{num2str(t0),'60000','1',num2str(inix),num2str(inix+dx*(m-1)),'1'});
    x(1)=eval(answ{1});
    x(2)=eval(answ{2});
    x(3)=eval(answ{3});
    y(1)=eval(answ{4});
