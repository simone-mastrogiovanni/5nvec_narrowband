function [gdout iok sbl_ dt0all phall phall1]=sfdb09_band2sbl(freq,cont,folder,pialist,filesbl,simpar,dtout)
...
%                     dfr0=sour1.df0*dt;
        ...
        %             sft=sft+sim';
                ...
                case 5
                    ...
                    ph1=(f0*(1-deinst)*tt+df0*tt/2.^2+ddf0*tt.^3/6)*2*pi;
        ...
        if dtout > 0 
            ...
            xfr(kfr)=sft;
            xt=2*ifft(xfr);
%             gdout=[gdout conj(xt(m4+1:3*m4))];

            gdout=[gdout xt(m4+1:3*m4)/red];        
%             if kkk == kcheck
%                 save('check1','xfr','sft','kfr');
%                 return
%             end         
        end
