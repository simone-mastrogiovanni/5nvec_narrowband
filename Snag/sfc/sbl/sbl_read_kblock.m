function [M,t,blhead]=sbl_read_kblock(sbl_,k,chn,indmin,indmax)
%READ_SBL   reads data from the k-th block of a sbl file 
%           the file should be opened at the beginning
%
%        see also sbl_read_block
%
...
lenbl=sbl_.blen;    
pos0=sbl_.point0+(k-1)*lenbl;

sta=fseek(sbl_.fid,pos0,'bof');
if sta < 0
    disp('end of file ');
    M=0;
    t=0;
...
for i = 1:nch
    ...
    switch typ
        ...
        case 5
            ...
            c=c(1:2:2*ndat(i))+j*c(2:2:2*ndat(i));
            M{i}=c.';
        case 6
            fseek(sbl_.fid,bias+(indmin(i)-1)*8,'bof');
