function [period perclean win perpar meanper]=gd2_period_enh(g2in,per,ph,nbin,subband,nofig)
% gd2_period  parallel epoch folding with a gd2
%                 use with show_gd2_per_par
%
%    [period perclean win perpar meanper]=gd2_period_enh(g2,per,ph,nbin,nofig)
% 
%   g2        input gd; or {g2 interv delay]; delay to add for real time
%   per       period (number,if string, time in mjd)
%                "day" "week" "sid" 
%   ph        phase (degrees); for per = 'sid', ph = antenna gives the loc sid time
...
switch icper
    case 1
        t=t-floor(t(1));
        t=mod(t+delay-ph/360,1);
        t2=mod(t+delay+interv-ph/360,1);
        nord=24;
    case 2
        t=(gmst(t)*15+long)/360;
        t=mod(t+delay*per,1);
        t2=mod(t+(delay+interv)*per,1);
        nord=24;
    case 3
        t=mod(t-5+delay,7)/7;
        t2=mod(t-5+delay+interv,7)/7;
        nord=7;
    ...
    case 4
        ...
        nord=24;
    otherwise
        t=mod((t+delay)/per-ph/360,1);
        t2=mod((t+delay+interv)/per-ph/360,1);
end

ii=floor((t+delay)*nbin)+1;
iii=find(ii == nbin+1);
ii(iii)=1;
ii2=floor((t+delay)*nbin)+1;
iii2=find(ii == nbin+1);
ii2(iii2)=1;

nbin1=round(nbin/2);
period=zeros(nbin+nbin1,m2-m1+1);
perclean=zeros(nbin,m2-m1+1);
win=period;
perpar=zeros(5,m2-m1+1);
jj=ii-ii2+1;
fprintf('%d points, covering %d bin;  ratio %f \n',length(jj),sum(jj),sum(jj)/length(jj))

for i = m1:m2
    for j = 1:n
        jjj=ii(j):ii2(j);
        period(jjj,i-m1+1)=period(jjj,i-m1+1)+M(j,i);
        win(jjj,i-m1+1)=win(jjj,i-m1+1)+abs(sign(M(j,i))); % 0s are holes
    end
end

period(1:nbin1,:)=period(1:nbin1,:)+period(nbin+1:nbin+nbin1,:);
period=period(1:nbin,:);
win(1:nbin1,:)=win(1:nbin1,:)+win(nbin+1:nbin+nbin1,:);
win=win(1:nbin,:);
nmax=max(max(win));
ic=0;
...
for i = m1:m2
    ...
    f=fft(period(:,im));
    f(numharm+2:nbin)=0;
    f(nbin:-1:nbin-numharm+1)=conj(f(2:numharm+1));%size(f),size(perclean),size(period)
    perclean(:,im)=ifft(f);
...
if nofig <= 0
    percl=y_gd2(perclean);
    percl=percl(:);
    ip=find(percl);
    percl=percl(ip);
    
    image_gd2(perclean,[min(percl) max(percl)]),title(strtit),xlabel('Hz')
    switch icper
        case 0
            ylabel('degrees')
        case 3
