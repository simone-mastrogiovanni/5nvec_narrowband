function [ener enwav eta psi AAA BBB]=find_max_polar(sour,ant,neta,npsi)
...
%
%   [ener eta psi]=find_max_polar(sour,ant,neps,npsi);
%
%  sour,ant   source and antenna structure
%  neta,npsi  number of eta and psis
...
for i = 1:neta
    ka=sqrt(1/(1+eta(i)^2));
    kb=sign(eta(i))*sqrt(eta(i)^2/(1+eta(i)^2));
    for j = 1:npsi
...
figure,image(psi,eta,ener1,'CDataMapping','scaled'),colorbar
xlabel('psi'),ylabel('eta')
figure,image(psi,eta,ener,'CDataMapping','scaled'),colorbar
xlabel('psi'),ylabel('eta')
