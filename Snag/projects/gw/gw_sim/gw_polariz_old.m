function g=gw_polariz_old(source,antenna,n,nsid,nper)
%GW_POLARIZ polarized source signal (to analyze the spectral dispersion)
%            (used by ana_polar)
%  
%   ATTENTION ! based on the old definition of the epsilon
%
...
for i = 1:n
    xc=cos(ang+fi)*sqrt(1-eps); 
    yc=sin(ang+fi)*sqrt(1-eps);
    xl=cos(ang)*sqrt(eps)*sin(2*psi);
    yl=cos(ang)*sqrt(eps)*cos(2*psi);
