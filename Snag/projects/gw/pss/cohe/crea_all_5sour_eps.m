function [sigl sigr asigl asigr]=crea_all_5sour_eps(hp,hx,neps,npsi)
...
%         cx=-1j*exp(1j*2*psi)*(1-eps)/sqrt(2);
        rot=1;
        lp=(eps+sqrt(2-eps.^2))/2*cos(2*psi);
        lx=(eps+sqrt(2-eps.^2))/2*sin(2*psi);
        cp=-1j*rot*(eps-sqrt(2-eps.^2))/2*sin(2*psi);
        cx=-1j*rot*(-eps+sqrt(2-eps.^2))/2*cos(2*psi);
        a=hp*(lp+cp)+hx*(lx+cx);
        sigl(i,j,:)=a;
