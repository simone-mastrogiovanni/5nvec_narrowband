...
sour=cand2sour(cand,proc_info.epoch);
sour=new_posfr(sour,proc_info.t0);
...
dsky=proc_info.fu.nsky*sqrt(patch(3).^2+(patch(4)-patch(5)).^2/4); % spot diameter
lam0=patch(1);
bet0=patch(2);
[smap,lam,bet]=pss_optmap_spot(ND,proc_info.fu.skyres,lam0,bet0,dsky/sqrt(2)); % sqrt for max error
spotstr.lam=lam;
spotstr.bet=bet;
spotstr.lam0=lam0;
spotstr.bet0=bet0;
...
nsdref=2*round(proc_info.fu.nsd*fftgain)+1;
frp1=-amaxt*(dsd*(nsdref+1)/2)-dfpos+frc;
frp4=amaxt*(dsd*(nsdref+1)/2)+dfpos+frc; frp{i}=[frp1 frp4];
...
job_info.proc.G_bsd_hspot.nsd=nsdref;
...
npea=length(ii);
bias=-frc+bias0;
% pea(2,:)=pea(2,:)-sdc*t(ii)-frc+bias0;
% pea(2,:)=pea(2,:)+bias;
pea1=pea0;
meanpea=[meanpea mean(pea0(2,:))]
% maxfp=max(pea(2,:)) 

[Tug1, kkp1,kkv1]=intersect(pea0(1,:),peaks_par.Tvel_pt);
nkk1=length(kkp1);
kkp1(nkk1+1)=length(pea0(1,:))+1;
...
for j = 1:kkkk
    for ll=1:nkk1
        pea1(2,kkp1(ll):kkp1(ll+1)-1)=pea0(2,kkp1(ll):kkp1(ll+1)-1)*(1-dvtot(j,ll))+skybias(j)+bias;
%         pea1(2,kkp1(ll):kkp1(ll+1)-1)=pea(2,kkp1(ll):kkp1(ll+1)-1)+dvtot(j,ll)*frc+skybias(j);
% %         pea1(2,kkp1(ll):kkp1(ll+1)-1)=peaks(2,ii(kkp(ll)):ii(kkp(ll+1))-1)+dvtot(ll)+skybias(j)+maxfp*(j-1); % SABRINA per prova poi quello sotto ATTENZIONE forse *sdc*t(ii
...
job_info.proc.G_bsd_hspot.bias=bias;
job_info.proc.G_bsd_hspot.bias0=bias0;
