function [spotstr,job_info,checkG]=bsd_hspot0(typ,candin,proc_info,job_info,peaks,patch)
...
% by O.Piccinni, S.D'Antonio, S. Frasca - sergio.frasca@roma1.infn.it
...
%         frc1=[];
    ...
    case 3
        ...
        lam0=patch(1);
        bet0=patch(2);
        [smap,lam,bet]=pss_optmap_spot(ND,proc_info.fu.skyres,lam0,bet0,dsky); disp('pss_optmap_spot'),size(smap),size(lam),size(bet),size(dvdir)
        refpat=smap(:,1:2)';
        dvlam=(lam-lam0).*dvdir(1,:)'; size(dvlam)
        dvbet=(bet-bet0).*dvdir(2,:)'; size(dvbet)
        maxadvlam=max(abs(dvlam(:)));maxadvlam
        maxadvbet=max(abs(dvbet(:)));maxadvbet
        maxadv=maxadvlam+maxadvbet;
        nl=length(lam);
