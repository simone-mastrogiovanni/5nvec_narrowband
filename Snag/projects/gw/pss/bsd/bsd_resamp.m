function [out,check_res]=bsd_resamp(in,direc,res)
...
%     t1=(i-1)*DT+DT/2;
    ...
    x=[last,x2,x1];
    x2=yyy(N4+1:N2); 
    tt=t1+(0:N2-1)*ddt+pbuf(1:N2)+toff+dtEinst;
    [xout, iout]=strobo(x,tt,dt,toff);iico=iico+1;ico(iico)=iout;
    nn=length(xout);
    if iout <= 0
        if -iout < nn
