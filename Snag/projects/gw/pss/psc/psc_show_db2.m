function [pars hfr hsky hsd]=psc_show_db2(fr,lam,bet,sd,cr,nsd,dirin)
%PSC_SHOW_DB2  create a candidate selection from a psc db2
%
%       [pars hfr hsky hsd]=psc_show_db2(fr,lam,bet,sd,cr,nsd,dirin)
%
%   fr           [frmin,frmax, nfr]; = 0 no selection; nfr = 0 -> standard
...
ncand=0;
nfr=infr2-infr1+1;
hx1=(1:infr2*100)*0.1; % da sistemare per sd

if icfr == 2
    nfr=fr(3);
    dfr=(fr(2)-fr(1))/(nfr-1);
    hfr=zeros(1,nfr);
    hx=fr(1):dfr:fr(2)*1.000001;
else
        ...
        if nn > 0
            ...
            if n2 > 0        
                ...
                for j = 1:nsd
                    ...
                    hsd(j,:)=hsd(j,:)+hist(cand1(4,jj),hx1);
                end
                i1=floor(cand1(2,:))+1;
                i2=floor(cand1(3,:))+91;
                i3=i1+i2*360;
