...
yn=is_bsd(cl_sosa);
if yn > 1
    disp('bsd_peakmap used !')
%     cont=ccont_gd(cl_sousa);
    
    tfstr=bsd_peakmap(cl_sosa,fftlen,thr);
...
function [C out]=modA(bsd_corrRun)

    C=ccont_gd(bsd_corrRun)
    [Nt ~]=size(C.v_eq);
    clear r r1 r2 r3 r4
    r(1:3,1:Nt)=0;
    r1(1:3,1:Nt)=0;
    ...
    for k=1:Nt
        sour1=sour;
        sour1=new_posfr(sour1,(C.t0+C.v_eq(k,4)/86400.0));
        r(:,k)=astro2rect([sour1.a sour1.d],0);
        r1(:,k)=astro2rect([sour1.a+1 sour1.d],0);
        r2(:,k)=astro2rect([sour1.a-1 sour1.d],0);
        r3(:,k)=astro2rect([sour1.a sour1.d+1],0);
        r4(:,k)=astro2rect([sour1.a sour1.d-1],0);
    end
    v_eq=(C.v_eq(:,1:3))';
    p_eq=(C.p_eq(:,1:3))';
    v1=dot(r,v_eq);
    ...
    vv(4,:)=dot(r4,v_eq);
    pars.cand=sour;
    pars.sour=new_posfr(sour,(C.t0)); % at the beg of the Run
    pars.limband=[C.inifr C.inifr+C.bandw]
    pars.appf0=sour1.f0;
    pars.df0=sour.df0;
    pars.time=adds2mjd(C.t0,C.v_eq(:,4)-1024); % Da controllare
    pars.v=v1;
    pars.p=p1;
    pars.vv=vv;
    pars.w=(C.v_eq(:,1:3))';
