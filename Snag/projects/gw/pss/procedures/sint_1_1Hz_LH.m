...
switch typ
    case 1
        mifr=10;
        fr=[33 42 65 91 141 290 512 812 1212 1812];
    case 2
        mifr=10;
        fr=[290 512 812 1212];
    case 3
...
%     Hsid(i)=mod(phcorr+par.hsid,24);
    ...
    snrsoleff(i)=par.asolmax/(meaper(i));
%     heff(i)=par.asidmax*hvirnois(i)/(2*meaper(i));
...
figure,loglog(fr,hnois),grid on,hold on,loglog(fr,hnois,'r.'),loglog(fr,snrsoleff.*hnois,'r'),loglog(fr,snrsoleff.*hnois,'b.')
title('Band noise (blue) and solar part (red)'),xlabel('Hz'),xlim([mifr mafr])
...
figure,loglog(fr,snrsoleff),grid on,hold on,loglog(fr,snrsoleff,'r.')
title('Fraction of the solar part'),xlabel('Hz'),xlim([mifr mafr])
