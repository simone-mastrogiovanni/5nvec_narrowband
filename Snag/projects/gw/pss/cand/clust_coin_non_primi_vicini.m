function coin=clust_coin(in1,in2,cand1,cand2,dist,numer,kepoch,reduce,nocref)
%
...
%   nocref      = 1 without cref (def 0; < 0 limit on m1 for check)
%
...
Sd1max=max(sd1max);
sd1=in1.clust(14,:);
fr1min=in1.clust(3,:)+kepoch*delay*Sd1min-Ufr1;
% dfr1min=max(max(-diff(fr1min)),0);
dfr1min=max(abs(kepoch*delay*sd1)+Ufr1);
fr1max=in1.clust(4,:)+kepoch*delay*Sd1max+Ufr1;
...
Sd2min=min(sd2min);
Sd2max=max(sd2max);
fr2min=in2.clust(3,:)-(1-kepoch)*delay*Sd2min-Ufr2;
% dfr2min=max(max(-diff(fr2min)),0);
dfr2min=max(abs((1-kepoch)*delay*sd2)+Ufr2);
fr2max=in2.clust(4,:)-(1-kepoch)*delay*Sd2max+Ufr2;
...
%         if fr2min(j) > fr1max(i)+dfr1min+dfr2min+frdist
        ...
        if fr2min(j)+lfr2+frdist < fr1min(i)  % Sabrina correction
            j1=j+1;
            continue
        end
