function [clust, serv, servcl, prog]=cand_clust_2(A,dist,dfr,dsd,adv)
...
%   adv      advanced procedure (0,1; default 1)
%
%
...
if adv > 0
    clust.step1_ncl=ncl;
    s=datestr(now);
    fprintf('start part 2 at %s  (advanced)\n',s)
    ...
    [aind,ind]=sort(index);
    [aa,ind1]=sort(ind); % ind inverso

    indind=zeros(1,ncl+1);
    ...
    f=f(ind);%size(f),size(indind),max(indind),min(indind)
    
    for i = 1:ncl
        ff=f(indind(i):indind(i+1)-1);
        servcl(2,i)=min(ff);
        servcl(3,i)=max(ff);
    end
    ...
    for i = ncl:-1:1
        while servcl(3,i)+frdist < servcl(2,j)
            j=j-1;
        end
        servcl(1,i)=j;
    end
...
%     figure,plot(indii(1,:),'.'),hold on,grid on,plot(indii(2,:),'r.')
    
    for i = 1:ncl
        if i == floor(i/50000)*50000
            s=datestr(now);
            fprintf('clust1 = %d %f %s\n',i,servcl(3,i),s)
        end
        icl1=ii(indii(1,i):indii(2,i));
        nn=length(icl1); % size(ii),max(indii(:))
