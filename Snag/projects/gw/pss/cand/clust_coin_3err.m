function coin=clust_coin_3(in1,in2,cand1,cand2,dist,numer,kepoch,reduce,m1lim)
%
% Coincidence 2014 format (C14) - abs beta coincidence
%
...
[AFR1MAX,IA1]=sort(FR1MAX);
[BFR2MIN,IB2]=sort(FR2MIN);
[dum,IIA1]=sort(IA1);
[dum,IIB2]=sort(IB2);
jmin=1;

for i = 1:m1
    while BFR2MIN(jmin) < AFR1MAX(i)-frdist & jmin < m2
        jmin=jmin+1;
    end
    MINJ(i)=jmin;
    if jmin == m2
...
[AFR1MAX,IA2]=sort(FR1MAX);
[BFR2MIN,IB1]=sort(FR2MIN);
[dum,IIA2]=sort(IA2);
[dum,IIB1]=sort(IB1);
jmax=m2;

for i = m1:-1:1
    while BFR2MIN(jmax) > AFR1MAX(i)+frdist & jmax > 1
        jmax=jmax-1;
    end
    MAXJ(i)=jmax;
    if jmax == 1
