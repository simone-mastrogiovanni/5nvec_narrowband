...
if exist('m1lim','var')
    m1=m1lim;
    m2=m1lim;
    fprintf('start 1 at %s  (limited)\n',datestr(now))
else
    fprintf('start 1 at %s \n',datestr(now))
end
...
for i = 1:m1
    while B2(jmin) < A1(i)-frdist & jmin < m2
        jmin=jmin+1;
    end
    MINJ(i)=jmin;
    if jmin == m2
...
for i = m1:-1:1
    while B1(jmax) > A2(i)+frdist & jmax > 1
        jmax=jmax-1;
    end
    MAXJ(i)=jmax;
    if jmax == 1
