...
%
%    cand(9,N)   [fr lam bet sd amp CR typ]
...
%         [ma,ima]=max(yy);
        ...
        if ma > robmed(i)
            ...
            cand(8,jj)=(job_info.patch(4)-job_info.patch(5))/4;
            cand(9,jj)=1;
            if mode == 2
                i1=max(ima-mno,1);
...
%                 [ma1,ima1]=max(yy);
                ...
                if abs(ima1-ima) > 2*mno
                    if ma1 > robmed(i)
                        ...
                        cand(7,jj)=job_info.patch(3)/2;
                        cand(8,jj)=(job_info.patch(5)-job_info.patch(4))/4;
                        cand(9,jj)=2;
                    end
                end
            end
