...
clear lines;
clear par;
MAXLOOP=10 %20   %number of loops around the true spin-down
...
% Hough differenziale: si parte da dmax0,dmin1, deltad,n_of_ff,deltaf 
% freq_tot,tim_tot,n_of_ff

    ispin=0;
    d=dmin1; 
    bin_df0=zeros(nbin_d,nbin_f0);  %%matrix for both spin-down and f_0
...
% Fine Houugh differenziale - Inizio Hough integrale
...
%  Fine Hough
    ...
    if abs(iloop) <= 1    %%Three figures for example 
        ...
        n=n_gd2(hdf0)/m;              %number of spin-down values
        n1=length(y);                  %comp 1 is number of frequencies
        i1=0:n-1;
        i2=0:m-1;
        x1=a1+i1*dx1;
        x2=a2+i2*dx2;
