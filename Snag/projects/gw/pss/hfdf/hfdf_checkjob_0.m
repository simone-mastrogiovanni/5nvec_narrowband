...
%     setpp=str2num(job_0(6:7));
    ...
    gmed=median(GS1);
    SP(nw0+1:nw0+wn)=gmed(nw1:nw2);
    nw0=nw0+wn;%%basic_info,size(peakt)
    peakt(1:basic_info.ntim,i)=basic_info.npeak_fft;
%%     peakt(1:2473,i)=basic_info.npeak_fft; %%
    ...
    tim(i)=basic_info.proc.A_read_peakmap.duration+...
        ...
        basic_info.proc.C_clean_peak_table.duration;
    
    timA(i)=basic_info.proc.A_read_peakmap.duration;
    timB(i)=basic_info.proc.B_crea_peak_table.duration;
    timC(i)=basic_info.proc.C_clean_peak_table.duration;
    clear(name);
end
...
spm=gd(SP);
spm=edit_gd(spm,'ini',wini,'dx',wdf);
...
figure,semilogy(fr,timC,'g'),hold on,grid on,semilogy(fr,timC,'g.')
semilogy(fr,timB,'r'),semilogy(fr,timB,'r.')
semilogy(fr,timA),semilogy(fr,timA,'b.')
title('pre-job times (A g, B r, C b)'),xlabel('frequency'),ylabel('s')
...
title('WSP: h density'),xlabel('Hz'),grid on

figure,semilogy(sqrt(spm)),hold on,semilogy(sqrt(spm),'r.')
title('Median spectrum: h density'),xlabel('Hz'),grid on
