function out=bp_par_estim2(h1,h2)
% parameter estimation for binary ps
%
%       out=bp_par_estim(h1,h2)
%
%   h1,h2   first and second harmonics (complex numbers)
...
for i = 1:50
    ...
    F(i)=abs(harm(1))/sqrt(abs(harm(1))^2+abs(harm(2))^2);
end

figure;plot(E,F);grid on
figure;plot(E(2:50),D(2:50));grid on

out.ecc=spline(F,E,abs(h1)/sqrt(abs(h1)^2+abs(h2)^2));
