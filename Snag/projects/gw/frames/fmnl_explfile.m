...
case 'FrAdcData'
   ...
   text1{2}=sprintf('Num --> Ch name --> Ndata --> SampTime --> Comp --> Type --> Position');
   text2{2}=text1{2};
   text1{3}='Null';
   text2{3}=text1{3};
   k=1;
   ...
   text1{k+2}=sprintf('%d --> %s --> %d --> %f --> %d --> %s  --> %d',i,char(oadcname{i}),ndata(i),dt(i),compress(i),...
         char(ctype{i}),posdata(i));
   k=k+1; 
   else
   text2{kk+2}=sprintf('%d --> %s --> %d --> %f --> %d --> %s  --> %d',i,char(oadcname{i}),ndata(i),dt(i),compress(i),char(ctype{i}), posdata(i));
   kk=kk+1;   
   end
   end
